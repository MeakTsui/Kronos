...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--pred-len", dest="pred_len", type=int, default=int(os.getenv("DEFAULT_PRED_LEN", "15")))
    p.add_argument("--samples", type=int, default=int(os.getenv("DEFAULT_SAMPLES", "1")))

    # explain params
    p.add_argument("--bins", type=int, default=int(os.getenv("EXPLAIN_BINS", "64")))
    ...
    p.add_argument("--sample-path", dest="sample_path", action="store_true" if _get_env_bool("EXPLAIN_SAMPLE_PATH", True) else "store_false")
    p.add_argument("--include-mode", dest="include_mode", action="store_true" if _get_env_bool("EXPLAIN_INCLUDE_MODE", True) else "store_false")
    p.add_argument("--include-expected", dest="include_expected", action="store_true" if _get_env_bool("EXPLAIN_INCLUDE_EXPECTED", True) else "store_false")
